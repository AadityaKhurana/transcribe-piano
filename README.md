...
Our neural network, defined in [streamlit_app.py](file:///Users/aadityakhurana/Desktop/Projects/transcribe_piano/streamlit_app.py#L14), is a PyTorch model based on the **"Onsets and Frames"** architecture. It breaks piano transcription into two easier tasks instead of trying to predict everything at once:
...
The training configuration is implemented in [train.ipynb](file:///Users/aadityakhurana/Desktop/Projects/transcribe_piano/train.ipynb):
...
To fix this, we use a custom [TranscriptionLoss](file:///Users/aadityakhurana/Desktop/Projects/transcribe_piano/train.ipynb#L91) that uses weighted Binary Cross-Entropy (BCE):
...
3. **Fallback Synth**: If FluidSynth is not installed on the user's computer, [streamlit_app.py](file:///Users/aadityakhurana/Desktop/Projects/transcribe_piano/streamlit_app.py) falls back to a custom math-based synthesizer. It generates pure sine waves for the active frequencies and applies a simple volume envelope (a fast linear volume ramp-up for the "attack" and a gradual fade-out for the "decay") so that it still sounds musical.
...
*   [streamlit_app.py](file:///Users/aadityakhurana/Desktop/Projects/transcribe_piano/streamlit_app.py): Streamlit web application. Contains the model architecture, audio preprocessing pipeline, decoding state machine, and synthesizer player.
*   [dataset.py](file:///Users/aadityakhurana/Desktop/Projects/transcribe_piano/dataset.py): PyTorch dataset class that reads, slices, and pads preprocessed `.npz` feature chunks for training.
*   [train.ipynb](file:///Users/aadityakhurana/Desktop/Projects/transcribe_piano/train.ipynb): Jupyter notebook containing the model training logic, custom loss weights, and learning rate scheduler.
*   [manual_testing.ipynb](file:///Users/aadityakhurana/Desktop/Projects/transcribe_piano/manual_testing.ipynb): Prototype notebook used for testing model inference step-by-step.
